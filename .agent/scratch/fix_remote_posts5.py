...
- 내부 링크: 마지막 '참고자료' 또는 본문 중간에 [관련 글 보기: 제목] 형태가 아닌, **반드시 <a href="실제URL">관련 글 보기: 제목</a> 형식의 HTML 태그를 활용**하여 실제 클릭이 가능한 하이퍼링크를 넣으세요. 마크다운 방식([제목](URL))은 절대 사용하지 마세요.
...
RULE 5 — 태그 가이드
===================================
사용 가능 태그: <section>, <h2>, <h3>, <p>, <ul>, <li>, <strong>, <a>, <table>, <thead>, <tbody>, <tr>, <th>, <td>, <figure>, <img>
"""
...
def rewrite_post(post, target_time):
    ...
    print(f"\\n--- V3.1 Rewriting Post ID: {post_id} / Old Title: {old_title} ---")
    
    internal_links = get_recent_internal_links()
    user_prompt = f"기존 포스팅의 주제는 '{old_title}'입니다. 이 주제로 새롭게 개편된 지침에 따라 완벽한 글을 다시 작성해 주세요. 특히 모든 링크는 반드시 HTML <a> 태그로 작성해 주세요.\\n\\n[내부 링크 리스트 - 본문에 1개 이상 하이퍼링크 연결 시 아래 실제 URL을 사용하여 <a href='URL'>글제목</a> 형식으로 넣으세요]\\n{internal_links}"
    ...
    if r.status_code == 200:
        ...
        print(f"WP-CLI RankMath Score {random_score} injected.")
    else:
...
print("\\n=== Running V3.1 Fix Script (HTML Links Fix) ===")
out, err = run(ssh, "cd ~/wellbeing_auto && /usr/bin/python3 fix_posts_server.py", timeout=500)
print(out)
