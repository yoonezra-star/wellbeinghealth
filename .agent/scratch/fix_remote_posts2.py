...
6. 전체 분량은 **반드시 공백 제외 2,000자 이상**으로 논문 수준으로 매우 길고 구체적으로 작성하세요.
7. 본문 중에 글자 수가 **400자 이상인 거대한 문단이 최소 2개 이상** 포함되어야 합니다.
...
def rewrite_post(post, target_time):
    ...
    user_prompt = f"기존 포스팅의 주제는 '{old_title}'입니다. 이 주제로 공백 제외 2000자 이상의 완벽하게 새롭고 구체적인 본문을 작성해 주세요. 문단 길이가 400자 이상인 큰 문단을 2개 이상 포함시켜주세요."
