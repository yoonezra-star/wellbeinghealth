...
6. 전체 분량은 **반드시 공백 제외 2,000자 이상(공백 포함 3,000자 이상)**으로 매우 길게 작성하세요.
7. 본문 내에 **각각 글자 수가 400자 이상인 매우 상세하고 뚱뚱한 문단(Block of text)이 최소 2개 이상** 포함되어야 합니다. 애드센스 승인 심사를 위해 텍스트 밀도가 매우 중요합니다.
8. 포커스 키워드를 본문 첫 문장 첫 단어로 배치하세요.
...
def rewrite_post(post, target_time):
    ...
    user_prompt = f"기존 포스팅의 주제는 '{old_title}'입니다. 이 주제로 공백 제외 2000자 이상의 완벽하게 새롭고 구체적인 본문을 작성해 주세요. 문단 길이가 400자 이상인 큰 문단을 2개 이상 확실하게 포함시켜주세요."
    ...
    if r.status_code == 200:
        ...
        # WP-CLI RankMath Update via CWD
        ...
        print(f"WP-CLI RankMath injected: Score {random_score}, Keyword '{focus_keyword}'. Verification result: {result.stdout.strip()}")
    else:
